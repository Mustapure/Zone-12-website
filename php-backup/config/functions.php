...
function registerUser($data) {
    ...
    // Assign role based on form data or membership type
    $role = 'user';
    if (isset($data['role']) && in_array($data['role'], ['user', 'vendor', 'admin'])) {
        $role = $data['role'];
    ...
    $stmt = $db->prepare("INSERT INTO users (first_name, last_name, email, password, phone, user_type, role, chapter_name, organization_name, verification_token) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
    if (!$stmt) {
...
function loginUser($email, $password) {
    ...
    $stmt = $db->prepare("SELECT id, first_name, last_name, email, password, user_type, role, email_verified FROM users WHERE email = ?");
